...
pip3 install -r config-editor/backend/requirements.txt

# 3. Setup Database
echo "[3/4] Setting up the database..."
export FLASK_APP=config-editor/backend/app.py
# This will create the migrations folder on first run
if [ ! -d "config-editor/backend/migrations" ]; then
    echo "Initializing migrations..."
    flask db init -d config-editor/backend/migrations
fi
echo "Applying database migrations..."
flask db upgrade -d config-editor/backend/migrations
...
echo "gunicorn --chdir config-editor/backend --bind 0.0.0.0:5000 app:app"
echo "Or using the provided systemd service file."
